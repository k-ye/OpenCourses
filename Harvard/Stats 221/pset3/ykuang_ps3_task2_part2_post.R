...
file.path = 'data/ps3_task2_part2_%s.rda'
file.suffix.list = c('SGD', 'ASGD', 'implicit', 'batch')
...
ggplot(data=plot.df, aes(x=t, y=log.loss)) + geom_line(aes(colour=label), size=1) + scale_x_log10(breaks=c(1e2,1e3,1e4,1e5)) + scale_y_log10()+ 
...
savePlot = FALSE
if (savePlot)
  ggsave(file='fig/ykuang_ps2_part2.png', width=16, heigh=9, dpi=300) 
