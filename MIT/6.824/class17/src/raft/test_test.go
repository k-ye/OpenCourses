...
func TestReElection2A(t *testing.T) {
	...
	TsPrintln(">>> Elect a leader")
	leader1 := cfg.checkOneLeader()
	TsPrintf("<<< CHECKPOINT: a leader=%d\n", leader1)

	// if the leader disconnects, a new one should be elected.
	TsPrintf(">>> Disocnnets old leader=%d\n", leader1)
	cfg.disconnect(leader1)
	newleader := cfg.checkOneLeader()
	TsPrintf("<<< CHECKPOINT: a new leader=%d after disconnection\n", newleader)

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	TsPrintf(">>> Old leader=%d rejoins\n", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	TsPrintf("<<< CHECKPOINT: old leader=%d rejoins, current leader=%d shouldn't change\n", leader1, leader2)

	// if there's no quorum, no leader should
	// be elected.
	TsPrintf(">>> Many peers down, no quorum\n")
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	TsPrintf("<<< CHECKPOINT: no quorum, no leader\n")

	// if a quorum arises, it should elect a leader.
	TsPrintf(">>> Some peers rejoin, quorum is about to be restored\n")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	TsPrintf("<<< CHECKPOINT: quorum restored, gets a leader\n")
