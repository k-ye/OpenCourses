...
func checkAppends(t *testing.T, v string, counts []int) {
	...
	for i := 0; i < nclients; i++ {
		...
		for j := 0; j < counts[i]; j++ {
			...
			if off < 0 {
				t.Fatalf("missing element in Append result, occured in i: %d, j: %d. Wanted: %s\n", i, j, wanted)
			}
			off1 := strings.LastIndex(v, wanted)
			if off1 != off {
				t.Fatalf("duplicate element in Append result, occured in i: %d, j: %d. Wanted: %s\n", i, j, wanted)
			}
			if off <= lastoff {
				t.Fatalf("wrong order for element in Append result, occured in i: %d, j: %d. Wanted: %s\n", i, j, wanted)
			}
			lastoff = off
		}
	}
