...
func TestBasic(t *testing.T) {
	...
	const nservers = 3
	// actual shardmaster servers
	var sma []*ShardMaster = make([]*ShardMaster, nservers)
	// shardmaster server names
	...
	// clerks know about shardmaster names for RPC
	ck := MakeClerk(kvh)
	...
	for i := 0; i < len(cfa); i++ {
		...
		// We can directly compare Shard because it is an Array with a fixed size,
		// think of it as a tuple in Python
