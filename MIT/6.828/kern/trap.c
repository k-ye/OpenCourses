...
extern void int_divide(void);
extern void int_debug(void);
extern void int_nmi(void);
extern void int_brkpt(void);
extern void int_oflow(void);
extern void int_bound(void);
extern void int_illop(void);
extern void int_device(void);
extern void int_dblflt(void);
											
extern void int_tss(void);
extern void int_segnp(void);
extern void int_stack(void);
extern void int_gpflt(void);
extern void int_pgflt(void);

extern void int_fperr(void);
extern void int_align(void);
extern void int_mchk(void);
extern void int_simderr(void);


extern void int_timer(void);
extern void int_kbd(void);
extern void int_serial(void);
// extern void int_spurious(void);
extern void int_ide(void);
...
trap_init(void)
{
	...
	// lgt[0] is null, cannot use it.
	// the last three bit of a selector is no part of the index to ldt!
	SETGATE(idt[T_DIVIDE], 0, GD_KT, int_divide, 0);
	SETGATE(idt[T_DEBUG], 0, GD_KT, int_debug, 0);
	SETGATE(idt[T_NMI], 0, GD_KT, int_nmi, 0);
	// Set DPL to 3 so that it could be used for user env
...
trap_init_percpu(void)
{
	...
	// (uint32_t)(percpu_kstacks[cpu_id] + KSTKSIZE)
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if ((tf->tf_cs & 3) == 0) {
		// panic("Kernel mode page fault, va: 0x%08x.\n", fault_va);
		cprintf("Kernel mode page fault, va: 0x%08x.\n", fault_va);
		return;
	}
	...
	if (curenv->env_pgfault_upcall == NULL) {
		...
		// Calculate the address of the user exception stack AFTER
		// pushing in the struct UTrapframe and the possible additional
		// one longword.
		uintptr_t uxstk_ptr = (tf->tf_esp < USTACKTOP) ? UXSTACKTOP : (tf->tf_esp - 4);
		uxstk_ptr -= sizeof(struct UTrapframe);
		// from this address up to UXSTACKTOP should all be PTE_P | PTE_U | PTE_W
		user_mem_assert(curenv, (const void *)uxstk_ptr, (UXSTACKTOP - uxstk_ptr), PTE_U | PTE_W | PTE_P);
		// Storing the utf at this address is equivalent to push onto the stack
		struct UTrapframe * utf = (struct UTrapframe *)uxstk_ptr;
		...
		// memcpy((void *)uxstk_ptr, &utf, sizeof(struct UTrapframe));
		tf->tf_esp = uxstk_ptr;
