...
// Description: this is the stress server application code. The server first
...
// creates a sockets and waits for connection from the client by calling
// srt_svr_sock() and srt_svr_connect(). It then receives the length of the file
// to be received. After that, it creates a buffer, receives the file data and
// saves the file data to receivedtext.txt file. Finally the server closes the
// socket by calling srt_server_close(). Overlay is stopped by calling
// overlay_end().
...
#define SVRPORT1 88
// after the received file data is saved, the server waits WAITTIME seconds, and
