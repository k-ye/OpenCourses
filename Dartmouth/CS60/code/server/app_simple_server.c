...
// Description: this is the simple server application code. The server first
// starts the overlay by creating a direct TCP link between the client and the
// server. Then it initializes the SRT server by calling srt_svr_init(). It
// creates 2 sockets and waits for connection from the client by calling
// srt_svr_sock() and srt_svr_connect() twice. The server then receives short
// strings sent from the client from both connections. Finally the server closes
// the socket by calling srt_server_close(). Overlay is stopped by calling
// overlay_end().
...
#define SVRPORT2 90
// after the strings are received, the server waits WAITTIME seconds, and then
