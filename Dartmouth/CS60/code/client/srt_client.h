...
// FILE: srt_client.h
//
// Description: this file contains client states' definition, some important
// data structures and the client SRT socket interface definitions. You need to
// implement all these interfaces
//
// Date: April 18, 2008
//       April 21, 2008 **Added more detailed description of prototypes fixed
//       ambiguities** ATC April 26, 2008 ** Added GBN and send buffer function
//       descriptions ** May 1, 2009    ** Clarified that srt_client_send is non
//       blocking ** ATC
//
...
// client transport control block. the client side of a SRT connection uses this
...
typedef struct client_tcb {
  ...
  pthread_mutex_t mu;           // mutex to protect the tcb, mainly |state|
...
//  example). Typically, the FSM has to be in a certain state determined by the
//  design of the FSM to carry out a certain action.
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
// the function call parameter client port.  The TCB table entry index should be
// returned as the new socket ID to the client and be used to identify the
// connection on the client side. If no entry in the TC table is available the
// function returns -1.
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
// Send data to a srt server. This function should use the SRT socket ID to find
// the TCP entry. It creates segBufs using the given data and append them to
// send linked list. If the send buffer is empty before insertion, a thread
// called sendbuf_timer should be started to poll the send buffer every
// SENDBUF_POLLING_INTERVAL time to check if a timeout event should occur. If
// the function completes successfully, it returns 1. Otherwise, it returns -1.
// srt_client_send is a non-blocking function call. Because user data is
// fragmented into fixed sized SRT segments there may be multiple segBufs queued
// to the send link list for a single srt_client_send call. If the call is
// successful the data is queued on the TCB send linked list and depending on
// the condition of the sliding window the data will either be trasmitted over
// the network or queued waiting to be transmitted.
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
// segments from the server. The design of seghanlder is an infinite loop that
...
// If the current time -  first sent-but-unAcked segment's sent time >
// DATA_TIMEOUT, a timeout event occurs When timeout, resend all
// sent-but-unAcked segments When the send buffer is empty, this thread
// terminates
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
