...
//
// Description: this is the simple client application code. The client first
// starts the overlay by creating a direct TCP link between the client and the
// server. Then it initializes the SRT client by calling srt_client_init(). It
// creates 2 sockets and connects to the server  by calling srt_client_sock() and
// srt_client_connect() twice. It then sends short strings to the server from
// these two connections. After some time, the client disconnects from the server
// by calling srt_client_disconnect(). Finally the client closes the socket by
// calling srt_client_close(). Overlay is stopped by calling overlay_end().
...
// SVRPORT1. The other uses client port CLIENTPORT2 and server port SVRPORT2
#define CLIENTPORT1 87
#define SVRPORT1 88
#define CLIENTPORT2 89
#define SVRPORT2 90

// after the strings are sent, wait WAITTIME seconds, and then close the
// connections
#define WAITTIME 5
...
int main() {
  ...
  // create a srt client sock on port CLIENTPORT2 and connect to srt server port
  // SVRPORT2
  ...
  sleep(WAITTIME);
  printf("client slepft for %d sec, begin to shutdown\n", WAITTIME);
